import { extend, isNode, filesListFrom, htmlToElement, bytesToSize } from "./utils";
...
export class Clippy {
    constructor(el, params) {
        ...
        if (this.params.activeContainerClass) {
            const clippy = this;

            this._isFilesListAdded = new Proxy(this._isFilesListAdded, {
                set(target, name, val, receiver) {
                    const ACTIVE_CLASS = clippy.params.activeContainerClass;
                    clippy.$container.classList[val === true ? "add" : "remove"](ACTIVE_CLASS);
                    return true;
                },
            });
        }
    ...
    _initInput() {
        ...
        if (this.params.accept && Array.isArray(this.params.accept)) {
            ...
            const acceptedExtensions = input.$el.getAttribute("accept");
            if (acceptedExtensions) {
                this.params.accept = acceptedExtensions.split(",").map((value) => value.trim());
            }
        }
    ...
    _createFilesList() {
        ...
        filesList.$el.addEventListener("click", (evt) => {
            ...
            if (filesList.$el.childNodes.length === 0) {
                filesList.$el.remove();
                this._isFilesListAdded.state = false;
            }
        });
    ...
    _addFilesList() {
        ...
        this._isFilesListAdded.state = true;
    }
    ...
    resetFiles() {
        const filesToRemove = this._getFilesList();
    ...
    _removeFile(evt) {
        ...
        if (this.userParams.filesList.items.remover) {
            closestButton = evt.target.closest(`.${REMOVER_PARAMS.classes[0]}`);
        } else {
            closestButton = evt.target.closest(`[data-clippy-remover]`);
        }
    ...
    _getFilesList() {
        ...
        const filesList = filesListFrom(files);
        return filesList;
    }

    _updateInputState() {
        this.input.$el.files = this._getFilesList();
        this.hasRemoveEvent = false;
    }
    ...
    _addMessageItems() {
        ...
        let direction = directionEl || this.$container;
    ...
    _checkFilesList() {
        if (!this._isFilesListAdded.state && this._filesToUpload.size > 0) {
            this._addFilesList();
        }
    }
